                                      ...
                                      main_address_(main_address)
{
    ...
    std::cout << "WORKER CLIENT HELLO AND GET REGION " << region_ << std::endl;
}
...
void ShortestPathsWorkerClient::send_to_neighbors()
{
    for (const auto region_info : neighbors_jobs_) {
        ...
        region_jobs.clear();
        std::cout << "END OF SENDING JOBS TO " << region_info.first << std::endl;
    }
}
